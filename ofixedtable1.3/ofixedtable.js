...
function oFixedTable(id, obj, _cfg){
	...
	_.ieLowVersion = userAgent.indexOf('MSIE 6.0') >= 0 || userAgent.indexOf('MSIE 7.0') >= 0;
		...
		if(!_.ieLowVersion){
			for(var i=0; i<_.config.rows; i++){
				...
				if(row != null){
					...
					rowClone.style.width = row.offsetWidth + 'px';
			...
			for(var i=0; i<_.config.rows; i++){
				...
				row.style.width = _.obj.rows[i].offsetWidth + 'px';
